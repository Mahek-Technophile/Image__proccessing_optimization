...
low_res_gray_image = cv2.resize(gray_image, (128, 128), interpolation=cv2.INTER_LINEAR)#calculates pixel value weeighted to average of nearest pixels
...
cv2.imshow("Normalized [0-1]", (normalized_image * 255).astype(np.uint8))#array to unsigned integear val
cv2.waitKey(0)
cv2.destroyAllWindows()
